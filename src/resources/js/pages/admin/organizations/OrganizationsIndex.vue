...
const { data: organizationsData, isFetching, isError, refetch } = useQuery({
    queryKey: ['admin-organizations'],
    queryFn: async () => {
        const response = await axios.get(organizations.index().url);
...
const columns = [
    ...
    columnHelper.display({
        id: 'actions',
        header: 'Actions',
        cell: ({ row }) => organizations.show(row.original.id).url,
    }),
];
...
const createUrl = organizations.showCreate().url;
    ...
    <div class="flex flex-col gap-4 p-4">
      ...
      <div class="flex flex-col gap-3 rounded-xl border border-sidebar-border/70 bg-muted/30 p-3 dark:border-sidebar-border">
        <div class="grid gap-3 md:grid-cols-3">
          <div class="space-y-1">
            <label class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Search</label>
            <input
                ...
                <template v-for="cell in row.getVisibleCells()" :key="cell.id">
                  <td class="px-4 py-3 align-middle">
                    <template v-if="cell.column.id === 'actions'">
                      <Link
                        :href="renderActionUrl(cell.getValue())"
                        class="inline-flex"
                      >
                        <Button
                          as="span"
